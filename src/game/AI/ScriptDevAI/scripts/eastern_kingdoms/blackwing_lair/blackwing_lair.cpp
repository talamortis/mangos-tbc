...
public:
    ...
    {
        return new instance_blackwing_lairAI(pMap);
    }

    struct instance_blackwing_lairAI : public ScriptedInstance
    {

        instance_blackwing_lairAI(Map* pMap) : ScriptedInstance(pMap),
            m_uiResetTimer(0),
            m_uiDefenseTimer(0),
        ...
        bool CheckConditionCriteriaMeet(Player const* pPlayer, uint32 uiInstanceConditionId, WorldObject const* pConditionSource, uint32 conditionSourceType) const
        {
            ...
            script_error_log("CheckConditionCriteriaMeet called with unsupported Id %u. Called with param plr %s, src %s, condition source type %u",
...
public:
    ...
    {
        return new go_suppressionAI(go);
    }


    struct go_suppressionAI : public GameObjectAI
    {
        go_suppressionAI(GameObject* go) : GameObjectAI(go), m_uiFumeTimer(urand(0, 5 * IN_MILLISECONDS)) {}
...
public:
    ...
    bool OnProcessEvent(uint32 uiEventId, Object* pSource, Object* /*pTarget*/, bool /*bIsStart*/) override
    {
        ...
        {
            if (instance_blackwing_lair::instance_blackwing_lairAI* pInstance = (instance_blackwing_lair::instance_blackwing_lairAI*)((Creature*)pSource)->GetInstanceData())
            {
                switch (uiEventId)
                {
                    // Left Chromaggus breath
